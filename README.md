...
## Overview
The multidimensional multicomponent FMM model (3DFMM$_k$) is adequate in scenarios where $k$ sequential events are observed from $d$ different directions. The 3DFMM$_k$ is described by a closed curve in 3D space, combining $k$ FMM curves lying in different planes, observed as projections on various directions.
